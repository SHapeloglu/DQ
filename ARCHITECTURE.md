...
- engine.py: CheckEngine + assert tipleri:
  Mevcut: not_empty, regex_match, accepted_values, freshness_hours, row_count_between, referential_integrity, equals, between, greater_than, less_than
  Backlog: custom_sql, schema_check, duplicate_row, volume_anomaly, statistical_anomaly
...
- config.py: SodaConfig + _resolve_env_vars() + referential_integrity
...
3. Kural ekle → /checks POST
4. Run tetikle → /api/run/{source_id} POST → api_post_run → alert

## 9. Puanlama (Rakip Karşılaştırma)
